...
test('copies text to clipboard and clear it', async () => {
...
test('handles errors when copying text', async () => {
  const { result } = renderHook(() => useCopyToClipboard())
...
test('returns false when clipboard API is not supported', async () => {
  const { result } = renderHook(() => useCopyToClipboard())
  const [, copy] = result.current
